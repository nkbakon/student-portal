    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-gray-100 overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                @if(auth()->user()->type == '1')
                <a href="{{ route('cashouts.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-900 focus:bg-blue-900 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-900 focus:ring-offset-2 transition ease-in-out duration-150">Add Cash Out</a><br><br>
                @endif
                    ...
                    <table class="w-full text-sm text-left text-gray-700 dark:text-gray-400">
                        ...
                        <tbody>
                            ...
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">                                    
                                ...
                                <td class="py-3 px-6">
                                    @if(isset($cashout->teacher))
                                        {{ $cashout->teacher->name }}
                                    @endif
                                </td>
                                <td class="py-3 px-6">
                                @if(isset($cashout->teacher))
                                    {{ $cashout->teacher->email }}
                                @endif
                                </td>
                                <td class="py-3 px-6">
                                    {{ $cashout->created_at->format('Y-m-d') }}
                                </td>
                                <td class="py-3 px-6">
                                    {{ $cashout->amount }}
                                </td>
    ...
    <div class="relative w-full h-full max-w-md md:h-auto">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-6 text-center">
                <form method="POST" action="{{ route('cashouts.destroy', 'data_id') }}">
                    @csrf
                    @method('DELETE')
